...
// based on Verilog uart_tx.v but only supports byte data
class UartTx {
    ...
    const uint8_t  DATAW = 8;  // only supports bytes
...
public:
    ...
    explicit UartTx(const UartConfig& cfg = {})
        : CNT_INC(cfg.cnt_inc),
...
int run(int argc, char* argv[], const SimConfig& config) {
    ...
    uart_tx = new UartTx(config.uart);
