...
async def pixel_colour(dut):
    ...
    await Timer((DISP_LINE-3)*SYS_TIME, units='ns')  # 3 cycles already when dy=0
    ...
    await Timer((DISP_LINE-1)*SYS_TIME, units='ns')  # 1 cycle already when dy=1
    ...
    await Timer((DISP_LINE-3)*SYS_TIME, units='ns')
