...
The **xd** [[verilog src](../sys/xd.v)] modules sends a flag (isolated pulse) from one clock domain to another. For example, the start of a frame is marked by [display timings](display_timings.md) in the _pixel clock_ domain; to use it in software we need it cross clock domains to the the _system_ clock domain.
